...
test('closed primary header has no account email or connection clutter', () => {
  ...
  assert.doesNotMatch(primary, />CAPTURE</);
  assert.doesNotMatch(primary, />NORMAL</);
  assert.doesNotMatch(primary, />EXPERT</);
  assert.doesNotMatch(primary, /PRESENTATION/);
  assert.doesNotMatch(primary, /MAP READY/);
