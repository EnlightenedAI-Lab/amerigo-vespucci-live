/**
 * Parse provider time strings and select nearest valid frame.
 */
export class TimeController {
  ...
  /**
   * @param {string[]} isoTimes UTC ISO strings from provider capabilities
   * @param {Date|string} [target] desired valid time; never hard-codes today
   */
  ...
  evidenceClassForTime(validIso, now = new Date()) {
    if (!validIso) return 'MODELLED';
    return new Date(validIso) > now ? 'FORECAST' : 'MODELLED';
  }
}

/**
 * Parse ISO-ish time from WMTS capabilities dimension values.
 * @param {string} value
 */
