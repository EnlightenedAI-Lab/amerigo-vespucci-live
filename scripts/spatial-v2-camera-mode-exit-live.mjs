/**
 * Live proof: Camera Mode chrome, BACK TO MAIN VIEW, directional camera glyph on :3052.
 */
...
const OUT = path.join(ROOT, 'artifacts', 'spatial-v2-camera-mode-exit-v1');
const PORT = 3052;
...
const SNAP = `(() => {
  ...
  const back = document.querySelector('[data-iqai-camera-back-main]');
  ...
  return {
    ...
    rings: visual?.querySelectorAll('[data-iqai-360-ring]').length || 0,
...
let live;
try {
  live = await withCdpPage(browserPath, 9422, async (send) => {
    const shots = {};
    ...
    shots.workspace = await shot(send, '01-workspace.jpg');
    ...
    await evaluateJson(send, `void window.__iqaiSpatialV2.cameraRelevance.query?.(true); true`).catch(() => null);
    await sleep(400);
    ...
    shots.viz = await shot(send, '02-camera-mode.jpg');
    ...
    shots.enlarged = await shot(send, '03-enlarged.jpg');
    await send('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
    await send('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
    await sleep(700);
    const restored = await evaluateJson(send, SNAP);
    shots.restored = await shot(send, '04-esc-3up.jpg');

    mark('back');
    await evaluateJson(send, `window.__iqaiSpatialV2.worldViewFrame.backToMainView(); true`);
    await sleep(800);
    const after = await evaluateJson(send, SNAP);
    shots.after = await shot(send, '05-back-main.jpg');
    await evaluateJson(send, `window.__iqaiSpatialV2.worldViewFrame.setLayout(2); true`);
    await sleep(400);
    const layoutAfter = await evaluateJson(send, SNAP);

    return { workspace, viz, enlarged, restored, after, layoutAfter, shots };
  });
} catch (error) {
...
const { workspace, viz, enlarged, restored, after, layoutAfter } = live;
const modePass = viz?.cameraViz === true && viz?.modeBarHidden === false && viz?.backVisible === true;
const disablePass = (viz?.layouts || []).every((btn) => btn.disabled === true)
  && (viz?.layouts || []).every((btn) => btn.title.includes('CAMERA MODE ACTIVE'));
const backPass = after?.cameraViz === false && after?.modeBarHidden === true;
const layoutPass = (after?.layouts || []).every((btn) => btn.disabled === false)
  && Number(layoutAfter?.layout) === 2;
const camerasPass = (after?.plannedCount || 0) >= 3;
const result = modePass && disablePass && backPass && layoutPass && camerasPass ? 'PASS' : 'PARTIAL';
