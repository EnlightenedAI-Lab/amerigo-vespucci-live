...
import { runMixedActionConformance } from '../tests/spatial/language/run-mixed-action-conformance.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.resolve(__dirname, '..');
const outDir = path.join(root, 'artifacts', 'language-conformance');

const result = runMixedActionConformance({ seed: 42 });
fs.mkdirSync(outDir, { recursive: true });
fs.writeFileSync(path.join(outDir, 'mixed-action-latest.json'), JSON.stringify(result, null, 2));

console.log('IQAI MIXED-ACTION LAYER COMPOUND SUPPLEMENTAL SUITE');
