 ...
 * Lab-local copy. Missing values stay null.
 */
...
export function wgs84ToUtm(latitude, longitude) {
  ...
  return { zone, band, hemisphere, easting, northing, epsg: hemisphere === 'N' ? 32600 + zone : 32700 + zone };
}
...
export function formatScale(scale) {
  const value = finiteNumber(scale);
...
export function describeCoordinates(latitude, longitude, extras = {}) {
  ...
  const lon = finiteNumber(longitude);
  if (lat == null || lon == null) return null;
  const digits = extras.ddDigits ?? 5;
  const utm = wgs84ToUtm(lat, lon);
  return {
    latitude: lat,
    longitude: lon,
    dd: formatDecimalDegrees(lat, lon, digits),
    dms: formatDms(lat, lon),
    utm: formatUtm(lat, lon),
    mgrs: formatMgrs(lat, lon),
    epsg: utm ? `EPSG:4326  ·  EPSG:${utm.epsg}` : 'EPSG:4326',
    elevation: formatElevationMeters(extras.elevationMeters),
    elevationMeters: finiteNumber(extras.elevationMeters),
    elevationSource: extras.elevationSource || null,
    place: extras.place || null,
    placeSource: extras.placeSource || null
  };
}
